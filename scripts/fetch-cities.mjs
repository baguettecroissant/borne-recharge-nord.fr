...
async function fetchCities() {
  const url = 'https://geo.api.gouv.fr/departements/59/communes?fields=nom,code,population,codesPostaux,centre';
  
  console.log('Fetching Nord (59) communes from Geo API...');
  try {
    ...
    // Process and filter communes in Nord (59) with population >= 2000
    const processed = data
      .map(c => {
        let lat = 50.6292; // Lille lat
        let lng = 3.0573;  // Lille lng
